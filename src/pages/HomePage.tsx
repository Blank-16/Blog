    ...
    <div className="w-full min-h-screen">
      ...
      <section className="border-b border-edge">
        <div className="max-w-5xl mx-auto px-6 py-16 md:py-24 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
          <div>
            <p className="text-[11px] tracking-[0.25em] uppercase text-muted mb-5">A writing space</p>
      ...
      {posts.length === 0 ? (
        <div className="max-w-5xl mx-auto px-6 py-32 text-center">
          ...
          <Link href="/add-post" className="text-sm border border-edge px-5 py-2.5 rounded-full text-ink transition-opacity hover:opacity-60">
        ...
        <>
          ...
          {featured && (
            <section className="border-b border-edge">
              <Link href={`/post/${featured.$id}`} className="group block max-w-5xl mx-auto px-6 py-12 md:py-16">
                <div className="md:grid md:grid-cols-[1fr_auto] md:gap-12 md:items-start">
                  <div>
                    ...
                    {featured.tags && featured.tags.length > 0 && (
                      <div className="flex gap-1.5 mb-4">
                        {featured.tags.slice(0, 2).map(tag => (
                          <span key={tag} className="text-[10px] uppercase tracking-widest px-2.5 py-0.5 border border-edge rounded-full text-muted">
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted">
                      ...
                      {featured.authorName && featured.$createdAt && <span className="opacity-30">·</span>}
                      {featured.$createdAt && <span>{formatDate(featured.$createdAt)}</span>}
                    </div>
                  </div>
                  {featured.featuredImage && (() => {
                    const url = appwriteService.getFilePreview(featured.featuredImage);
                    return url ? (
                      <div className="relative mt-8 md:mt-0 md:w-64 lg:w-80 aspect-[4/3] rounded-xl overflow-hidden flex-shrink-0">
                        <Image src={url.toString()} alt={featured.title} fill priority
          ...
          {rest.length > 0 && (
            <section className="max-w-5xl mx-auto px-6 py-12">
              <div className="flex items-center justify-between mb-8">
                ...
                <Link href="/all-posts" className="text-xs text-muted underline underline-offset-4 transition-opacity hover:opacity-60">
                  View all →
                </Link>
              </div>
              <HomeGrid posts={rest} />
